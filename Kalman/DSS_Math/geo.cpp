...
double get_mag_declination(double lat, double lon)
{
	...
	int min_lat = static_cast<int>((lat / SAMPLING_RES) * SAMPLING_RES);
	int min_lon = static_cast<int>((lon / SAMPLING_RES) * SAMPLING_RES);
	...
	if (lat <= SAMPLING_MIN_LAT) {
		min_lat = static_cast<int>(SAMPLING_MIN_LAT);
	}

	if (lat >= SAMPLING_MAX_LAT) {
		min_lat = static_cast<int>((lat / SAMPLING_RES) * SAMPLING_RES - SAMPLING_RES);
	}

	if (lon <= SAMPLING_MIN_LON) {
		min_lon = static_cast<int>( SAMPLING_MIN_LON);
	}

	if (lon >= SAMPLING_MAX_LON) {
		min_lon = static_cast<int>((lon / SAMPLING_RES) * SAMPLING_RES - SAMPLING_RES);
	}

	/* find index of nearest low sampling point */
	unsigned min_lat_index = static_cast<unsigned>((-(SAMPLING_MIN_LAT) + min_lat)  / SAMPLING_RES);
	unsigned min_lon_index = static_cast<unsigned>((-(SAMPLING_MIN_LON) + min_lon) / SAMPLING_RES);
					...
					double *lat_target, double *lon_target)
{
	if (fabs(dist) < FLT_EPSILON) {
		...
		*lon_target = lon_A;
		...
		double heading = get_bearing_to_next_waypoint(lat_A, lon_A, lat_B, lon_B);
		waypoint_from_heading_and_distance(lat_A, lon_A, heading, dist, lat_target, lon_target);

	} else {
		double heading = get_bearing_to_next_waypoint(lat_A, lon_A, lat_B, lon_B);
		heading = _wrap_2pi(heading + M_PI);
		waypoint_from_heading_and_distance(lat_A, lon_A, heading, dist, lat_target, lon_target);
	}
}
...
double get_bearing_to_next_waypoint(double lat_now, double lon_now, double lat_next, double lon_next)
{
	...
	/* conscious mix of double and double trig function to maximize speed and efficiency */
	double theta = atan2(sin(d_lon) * cos(lat_next_rad) ,
			...
			double radius, double arc_start_bearing, double arc_sweep)
{
	...
	double	bearing_sector_start;
	double	bearing_sector_end;
	double	bearing_now = get_bearing_to_next_waypoint(lat_now, lon_now, lat_center, lon_center);
	bool	in_sector;
	...
	if (arc_sweep >= 0.0f) {
		...
		if (bearing_sector_end > 2.0f * M_PI) { bearing_sector_end -= M_TWOPI_F; }
	...
	if (in_sector) {
		crosstrack_error->past_end = false;
		double dist_to_center = get_distance_to_next_waypoint(lat_now, lon_now, lat_center, lon_center);
		...
		double *dist_xy, double *dist_z)
{
	...
	double dxy = CONSTANTS_RADIUS_OF_EARTH * c;
	double dz = alt_now - alt_next;
...
double mavlink_wpm_distance_to_point_local(double x_now, double y_now, double z_now,
		...
		double *dist_xy, double *dist_z)
{
	double dx = x_now - x_next;
	double dy = y_now - y_next;
	double dz = z_now - z_next;
